...
//    button.backgroundColor = button.selected ? self.manager.configuration.themeColor : nil;
    [button setBackgroundImage:[HXPhotoTools hx_imageNamed:@"hx_compose_guide_check_box_default@2x.png"] forState:UIControlStateNormal];
    [button setBackgroundImage:[HXPhotoTools hx_imageNamed:@"hx_original_selected@2x.png"] forState:UIControlStateSelected];
    ...
    if (self.manager.configuration.singleSelected) {
        if (model.type == HXPhotoModelMediaTypeVideo ) {
            if (model.asset.duration > self.manager.configuration.videoMaxDuration) {
                [self.view showImageHUDText: @"视频过大,无法选择"];
                return;
            }else if (model.asset.duration < 3.f) {
                [self.view showImageHUDText: @"视频少于3秒,无法选择"];
                return;
            }
...
- (void)deleteClick {
    if (!self.modelArray.count) {
        [self.view showImageHUDText:@"当前没有可删除的资源"];
        return;
    }
    NSString *message;
    if (self.currentModel.subType == HXPhotoModelMediaSubTypePhoto) {
    ...
    HXWeakSelf
    hx_showAlert(self, message, nil, @"取消", @"删除", ^{
...
- (UIButton *)darkDeleteBtn {
    if (!_darkDeleteBtn) {
        ...
        [_darkDeleteBtn setTitle:@"删除" forState:UIControlStateNormal];
...
- (UINavigationItem *)navItem {
    if (!_navItem) {
        ...
        if (self.previewShowDeleteButton) {
            _navItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(dismissClick)];
            _navItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"删除" style:UIBarButtonItemStylePlain target:self action:@selector(deleteClick)];
        }else {
            _navItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(dismissClick)];
        }
        if (self.exteriorPreviewStyle == HXPhotoViewPreViewShowStyleDefault) {
