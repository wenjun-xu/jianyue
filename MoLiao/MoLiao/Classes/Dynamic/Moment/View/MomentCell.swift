...
@objc protocol MomentCellDelegate {
    ...
    // 选择评论
    @objc optional func didSelectComment(cell: MomentCell,model:Comment)
    // 点击高亮文字
...
class MomentCell: UITableViewCell, MLLinkLabelDelegate {
    ...
    var moment: Moment! {
        ...
        didSet {
            ...
            headImageView?.image = UIImage(named: moment.userThumbPath!)
                ...
                for i in 0..<count  {
                    ...
                    lab.didClickTextBlock = { (comments: Comment) in
                        ...
                        if self.delegate != nil {
                            self.delegate?.didSelectComment!(cell: self, model: lab.comment)
                        }
                    }
    ...
    func addContentView() {
        ...
        // 名字视图
        nameLab = UILabel(frame: CGRect(x: (headImageView?.right)! + 10, y: (headImageView?.top)!, width: 60, height: 20.0))
