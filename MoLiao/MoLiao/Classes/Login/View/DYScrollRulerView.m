...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF                          = [[UITextField alloc]initWithFrame:CGRectMake(self.bounds.size.width/2-50, 10,100, 40)];
...
-(UILabel *)unitLab{
    if (!_unitLab) {
        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( self.bounds.size.width/2+25 , 10, 40, 40)];
        _unitLab.textColor = [UIColor grayColor];
    }
    return _unitLab;
}
