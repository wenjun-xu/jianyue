...
- (void)goCameraViewController {
    if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        [[self hx_viewController].view showImageHUDText:@"无法使用相机!"];
        return;
    }
...
- (void)cameraDidNextClick:(HXPhotoModel *)model {
    ...
    if (model.type == HXPhotoModelMediaTypeCameraPhoto) {
        ...
        if ([self.manager afterSelectPhotoCountIsMaximum]) {
            [[self hx_viewController].view showImageHUDText:[NSString stringWithFormat:@"最多只能选择%ld张图片",self.manager.configuration.photoMaxNum]];
            return;
        }
    }else if (model.type == HXPhotoModelMediaTypeCameraVideo) {
        // 当选中视频个数没有达到最大个数时就添加到选中数组中 
        if (model.videoDuration < 3) {
            [[self hx_viewController].view showImageHUDText:@"视频少于3秒,无法选择"];
            return;
        }else if (model.videoDuration > self.manager.configuration.videoMaxDuration) {
            [[self hx_viewController].view showImageHUDText:@"视频过大,无法选择"];
            return;
        }else if ([self.manager afterSelectVideoCountIsMaximum]) {
            [[self hx_viewController].view showImageHUDText:[NSString stringWithFormat:@"最多只能选择%ld个视频",self.manager.configuration.videoMaxNum]];
            return;
        }
    }
