...
- (void)setupUI {
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"导航@2x.png"] forBarMetrics:UIBarMetricsDefault];
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
                    ...
                    if (weakSelf.manager.configuration.replaceCameraViewController) {
                        ...
                        weakSelf.manager.configuration.useCameraComplete = ^(HXPhotoModel *model) {
                            if (model.videoDuration > weakSelf.manager.configuration.videoMaxDuration) {
                                [weakSelf.view showImageHUDText:@"视频过大,无法选择"];
                            }
        ...
        if (cell.model.isICloud) {
            if (self.manager.configuration.downloadICloudAsset) {
                ...
                [self.view showImageHUDText:@"尚未从iCloud上下载，请至系统相册下载完毕后选择"];
            }
            return;
        }
        if (!self.manager.configuration.singleSelected) {
...
//    selectBtn.backgroundColor = selectBtn.selected ? bgColor : nil;
    [selectBtn setBackgroundImage:[HXPhotoTools hx_imageNamed:@"hx_original_selected@2x.png"] forState:UIControlStateSelected];
...
//    self.selectBtn.backgroundColor = model.selected ? self.selectBgColor :nil;
    [self.selectBtn setBackgroundImage:[HXPhotoTools hx_imageNamed:@"hx_compose_guide_check_box_default@2x.png"] forState:UIControlStateNormal];
    [self.selectBtn setBackgroundImage:[HXPhotoTools hx_imageNamed:@"hx_original_selected@2x.png"] forState:UIControlStateSelected];
...
- (void)downloadError:(NSDictionary *)info {
    if (![[info objectForKey:PHImageCancelledKey] boolValue]) {
        [[self hx_viewController].view showImageHUDText:@"下载失败，请重试！"];
    }
    self.downloadView.hidden = YES;
...
- (UIToolbar *)bgView {
    if (!_bgView) {
        ...
        [_bgView setBackgroundImage:[UIImage imageNamed:@"导航@2x.png"] forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
    }
    return _bgView;
}
- (UILabel *)subTitleLb {
...
- (void)setVideoCount:(NSInteger)videoCount {
    ...
    if (self.photoCount > 0 && videoCount > 0) {
        ...
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@",videoCount,videoStr];
    }
}
- (void)layoutSubviews {
    [super layoutSubviews];
...
//    self.doneBtn.backgroundColor = [UIColor whiteColor];
    self.doneBtn.backgroundColor = [UIColor greenColor];
        ...
        if (self.manager.configuration.doneBtnShowDetail) {
            if (!self.manager.configuration.selectTogether) {
                if (self.manager.selectedPhotoCount > 0) {
                    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",@"完成",selectCount,self.manager.configuration.photoMaxNum] forState:UIControlStateNormal];
                }else {
                    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",@"完成",selectCount,self.manager.configuration.videoMaxNum] forState:UIControlStateNormal];
                }
            }else {
                [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",@"完成",selectCount,self.manager.configuration.maxNum] forState:UIControlStateNormal];
            }
        }else {
            [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld)",@"完成",selectCount] forState:UIControlStateNormal];
        }
    }
    self.doneBtn.backgroundColor = self.doneBtn.enabled ? [UIColor greenColor] : [[UIColor greenColor] colorWithAlphaComponent:0.5];
